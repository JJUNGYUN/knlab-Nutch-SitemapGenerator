  ...
  private class FetcherThread extends Thread {
    ...
    public void run() {
      ...
      try {
        ...
        while (true) {
		  ...
		  if(null == fit.url || fit.url.contains(" ")) {
			LOG.error("Invalid Url. This Url is null or contains space characters. fit.url : " + fit.url);
			return;
		  } 
    ...
    private void handleRedirect(String url, String newUrl, boolean temp, String redirType, WebPage page) throws URLFilterException, IOException, InterruptedException {
      ...
      if (reprUrl == null) {
		...
		// reprUrl length prevention
		if(reprUrl.length() > 767) {
			LOG.info("reprUrl is too long.\nreprUrl : " + reprUrl + "\nlength : " + reprUrl.length());
			reprUrl = reprUrl.substring(0, 767);
		}
    ...
    throws IOException, InterruptedException {
      ...
      if (content != null) {
		...
		// contentType length prevention
		if(contentType.length() > 32) {
			LOG.info("contentType(typ) is too long.\ncontentType : " + contentType + "\nlength : " + contentType.length());
			contentType = contentType.substring(0, 32);
		}	

        fit.page.setContent(ByteBuffer.wrap(content.getContent()));
        fit.page.setContentType(new Utf8(contentType));

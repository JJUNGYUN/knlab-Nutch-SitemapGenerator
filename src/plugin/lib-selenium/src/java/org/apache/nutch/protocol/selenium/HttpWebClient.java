					            ...
					            profile.setPreference("permissions.default.stylesheet", 2);
						          profile.setPreference("permissions.default.image", 2);
							        profile.setPreference("dom.ipc.plugins.enabled.libflashplayer.so", "false");
								      profile.setPreference("webdriver_firefox_allowed_hosts", "localhost");
																				          	...
																				          	int loadStylesheet = conf.getInt("selenium.firefox.load.stylesheet", 1);
																						    		    FirefoxProfile profile = new FirefoxProfile();
																								        		    FirefoxBinary binary = new FirefoxBinary();
																											        		    profile.setPreference("webdriver_firefox_allowed_hosts", allowedHost);
												        ...
												        if (!fs.exists(screenshotPath)) {
													        ...
													        InputStream is = new BufferedInputStream(new FileInputStream(srcFile));
														        IOUtils.copyBytes(is, os, conf);
															        LOG.debug(String.format("Screenshot for {} successfully saved to: {} {}", url, screenshotPath, srcFile.getName())); 
																      } else {
